...
public class LutPanelDialog extends DialogFragment {
    ...
    @NonNull @Override public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        return new AlertDialog.Builder(requireActivity())
                .setTitle("LUT 调色 - " + LutSetting.getSummary())
                .setView(recycler)
                .setNeutralButton("重置", (dialog, which) -> { LutSetting.reset(); dismiss(); })
    ...
    private class PresetAdapter extends RecyclerView.Adapter<PresetAdapter.ViewHolder> {
        ...
        @Override public void onBindViewHolder(@NonNull ViewHolder holder, int position) { LutPreset preset = mPresets.get(position); holder.textView.setText(preset.getName()); holder.textView.setOnClickListener(v -> { LutSetting.setEnabled(true); LutSetting.setPresetId(preset.getId()); dismiss(); }); }
