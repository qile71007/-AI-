...
import com.fongmi.android.tv.ui.dialog.SiteSpeedDialog;
...
public class SettingFragment extends BaseFragment implements ConfigListener, SiteListener, LiveListener, ThemeDialog.Listener {
    ...
    protected void initEvent() {
        ...
        mBinding.versionText.setOnClickListener(this::onVersion);
        ...
        // ==================== 新增：长按版本号弹出私密配置生成工具 ====================
        mBinding.versionText.setOnLongClickListener(v -> {
            showEncryptTool();
            return true;
        });
    }
    ...
    private void showEncryptTool() {
        View view = LayoutInflater.from(requireContext()).inflate(R.layout.dialog_encrypt, null);
        ...
        btnGenerate.setOnClickListener(v -> {
            ...
            if (namePlain.isEmpty() || urlPlain.isEmpty() || keyPlain.isEmpty()) {
                Toast.makeText(getContext(), "名称、URL、关键词不能为空", Toast.LENGTH_SHORT).show();
                return;
            }
            try {
                ...
                Toast.makeText(getContext(), "JSON生成完成", Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
        ...
        btnCopy.setOnClickListener(v -> {
            if (finalJson[0].isEmpty()) {
                Toast.makeText(getContext(), "请先生成JSON", Toast.LENGTH_SHORT).show();
                return;
            }
            ClipboardManager clipboard = (ClipboardManager) requireContext().getSystemService(Context.CLIPBOARD_SERVICE);
            ClipData clipData = ClipData.newPlainText("私密配置", finalJson[0]);
            clipboard.setPrimaryClip(clipData);
            Toast.makeText(getContext(), "JSON已复制到剪贴板，粘贴到urls数组即可", Toast.LENGTH_SHORT).show();
        });
